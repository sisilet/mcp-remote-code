...
import { checkHostKey, mismatchMessage, recordHostKey, unknownHostMessage, } from "./known-hosts.js";
// Re-exported for tests: these encode the (code, signal) contract.
export { exitCodeFrom, appendSignal } from "./ssh-pool.js";
import { BoundedBuffer, DEFAULT_MAX_OUTPUT_BYTES, buildAlgorithms, detectKeyType, intFromEnv, exitCodeFrom, appendSignal, isConnectionError, parseOpenSshOption, } from "./ssh-pool.js";
/**
...
export async function createSSHConnection(config, deps = {}) {
    ...
    let client = await openConnection(config);
    let closed = false;
    ...
    async function getSftp() {
        if (sftpUnavailable)
            throw new Error("SFTP subsystem unavailable on this server");
        if (sharedSftp)
            return sharedSftp;
        const c = await ensureClient();
        sharedSftp = await new Promise((resolve, reject) => {
            c.sftp((err, sftp) => {
                if (err) {
                    if (/unable to start subsystem/i.test(err.message)) {
                        sftpUnavailable = true;
                        console.error(`[SSH] ${config.host}: no SFTP subsystem; file transfers will use the ` +
    ...
    return {
        async exec(command, options = {}) {
            ...
            return execWithRetry(() => withChannel(async () => {
                const c = await ensureClient();
        ...
        async execStream(command, options = {}) {
            ...
            return new Promise((resolve, reject) => {
                c.exec(command, (err, stream) => {
                    ...
                    stream.stderr.on("data", (d) => errBuf.push(d));
                    ...
                    const done = new Promise((res) => {
                        ...
                        const timer = options.timeout
                            ...
                            : undefined;
                        stream.on("close", (code, signal) => finish(exitCodeFrom(code, signal), appendSignal(errBuf.toString(), signal)));
                        // Without this, an errored stream never closes, `done` never
                        // settles, and the slot is never returned.
                        stream.on("error", (e) => finish(-1, e.message));
                    });
                    resolve({
        ...
        async withSftp(fn) {
            ...
            const run = async () => {
                ...
                try {
                    ...
                    if (isChannelOpenFailure(err) || isConnectionError(err)) {
                        sharedSftp = undefined;
                        try {
                            sftp.end();
                        }
...
export function isChannelOpenFailure(err) {
 ...
 */
export async function execWithRetry(attempt, options = {}) {
    ...
    for (;;) {
        try {
            ...
            if (isChannelOpenFailure(error)) {
                channelOpenAttempts++;
                if (channelOpenAttempts >= MAX_CHANNEL_OPEN_ATTEMPTS)
                    throw error;
            ...
            if (isConnectionError(error)) {
                ...
                connectionAttempts++;
                if (options.retry !== true || connectionAttempts >= MAX_CONNECTION_ATTEMPTS)
                    throw error;
                continue;
            }
            throw error;
...
function warnOnDeprecatedPoolEnv() {
    for (const name of ["REMOTE_POOL_COMMAND_SIZE", "REMOTE_POOL_FILE_SIZE", "REMOTE_POOL_STAGGER_MS"]) {
        if (process.env[name]) {
            console.error(`[SSH] ${name} is obsolete: there is now one connection per target with ` +
...
function connect(config) {
    return new Promise((resolve, reject) => {
        ...
        if (policy === "insecure") {
            console.error(`[SSH] WARNING: host key verification disabled for ${config.host}:${config.port}.`);
            ...
            connConfig.hostVerifier = (key) => {
                ...
                if (policy === "accept-new") {
                    try {
                        ...
                        console.error(`[SSH] Recorded new host key for ${config.host}:${config.port} (${result.fingerprint})`);
                        return true;
                    }
                    catch (err) {
                        hostKeyError = new Error(`Could not record host key: ${err.message}`);
                        return false;
                    }
                }
...
function execOnChannel(client, command, timeoutMs, stdin, maxOutputBytes) {
    return new Promise((resolve, reject) => {
        ...
        const timer = timeoutMs && timeoutMs > 0
            ? setTimeout(() => {
                ...
                const err = new Error(`Remote Code: SSH exec timeout after ${timeoutMs}ms`);
                err.isTimeout = true;
                reject(err);
            }, timeoutMs)
            : undefined;
        client.exec(command, (err, stream) => {
            ...
            stream.on("data", (d) => outBuf.push(d));
            stream.stderr.on("data", (d) => errBuf.push(d));
