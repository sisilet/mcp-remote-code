...
export async function requireConnection(connectionManager, target) {
    ...
    if (target) {
        ...
        if (offlineError) {
            ...
            return {
                errorText: `Target "${target}" is offline: ${offlineError}. ` +
                    `Connected targets: ${available.length > 0 ? available.join(", ") : "(none)"}. ` +
                    retryHint,
            };
        }
        return {
            errorText: `Target "${target}" not found. Available targets: ${available.length > 0 ? available.join(", ") : "(none)"}`,
        };
    }
    if (targets.length === 0) {
        const offlineHint = offline.length > 0
            ? ` Offline: ${offline.map((f) => `${f.name} (${f.error})`).join("; ")}. ` +
                `Call a tool with target=<name> to retry a specific host.`
            : "";
        return {
            errorText: "No remote connections available. Start the server with --config or --ssh/--path." +
                offlineHint,
        };
    }
    if (targets.length > 1) {
 ...
 */
export async function jailRemoteDir(conn, input) {
 ...
 */
export function keepUnderRoot(conn, paths) {
