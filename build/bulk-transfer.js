 ...
 */
export async function pullDirectoryViaTar(sshPool, remoteDir, localDir) {
    ...
    const extractDone = new Promise((resolve) => {
        extract.on("close", (code) => resolve(code ?? 0));
    });
    try {
        await pipeline(stdout, extract.stdin);
    }
    catch (err) {
        ...
        return { ok: false, error: `stream failed: ${err.message}` };
    }
 ...
 */
export async function pushDirectoryViaTar(sshPool, localDir, remoteDir) {
    ...
    try {
        ...
        await pipeline(create.stdout, stdin, { end: false });
