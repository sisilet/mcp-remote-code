...
export function createRemoteStatTool(server, connectionManager) {
    server.registerTool("remote_stat", {
...
`;
        ...
        if (type === "symlink") {
            ...
            output.push(`Size: ${fields[1] || "0"} bytes`, `Modified (epoch): ${fields[2] || "0"}`, `Birth (epoch): ${fields[3] || "0"}`, `Mode (octal): ${fields[4] || "0"}`);
        }
        return textResult(output.join("\n"));
    });
}
//# sourceMappingURL=remote-stat.js.map