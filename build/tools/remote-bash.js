...
export function evaluateBashExecution(root, cwd, outside) {
    let actualCwd = root;
...
export async function handleRemoteBash(connectionManager, args) {
    ...
    const outcome = checkConfirmation(confirmationKey, needsOutsideConfirm, force, () => [
    ...
    if (needsOutsideConfirm) {
        ...
        const mode = resolveMode(conn.config.elicitation);
        ...
        if (verdict.via === "user-declined") {
            return textResult(`Declined by the user. The command was not run.\n\nCommand: ${command}`);
        }
        if (verdict.via === "error") {
            return textResult(`Could not obtain user confirmation (${verdict.detail}). The command was not run.`);
        }
    ...
    async function runCommand() {
        ...
        if (result.exitCode !== 0) {
            ...
            return textResult(`Command failed with exit code ${result.exitCode}:\n${command}\n\n${message}`);
        }
        let output = stdout;
        if (stderr.trim()) {
            output += "\n\nstderr:\n" + stderr;
