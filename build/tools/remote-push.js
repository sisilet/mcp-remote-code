...
export function createRemotePushTool(server, connectionManager) {
    server.registerTool("remote_push", {
        ...
        if (stat.type === "unsupported") {
            return textResult(`Unsupported local path type: ${localPath}. remote_push supports regular files and directories.`);
        }
        const plan = {
            localPath,
            remotePath,
...
async function scanLocalDirectory(localPath) {
...
async function pushFile(sshPool, localPath, remotePath) {
...
async function pushDirectory(sshPool, localPath, remotePath) {
    const tree = await scanLocalDirectory(localPath);
...
function remoteChildPath(localBase, localChild, remoteBase) {
...
//# sourceMappingURL=remote-push.js.map