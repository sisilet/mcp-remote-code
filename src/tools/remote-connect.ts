...
export function createRemoteConnectTool(server: McpServer, connectionManager: ConnectionManager) {
  server.tool(
    "remote_connect",
    "Connect to a remote machine via SSH. You can connect by referencing a saved config (use remote_list_configs to see available configs) or by providing connection details directly.",
    {
    ...
    async ({ machine, ssh, workdir, password, sudo_password }) => {
      try {
        ...
        if (config) {
          ...
          info = await connectionManager.connectWithParams(
            machine,
            ssh,
            workdir,
            password,
