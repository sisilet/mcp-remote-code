...
function describeLauncher(): string {
...
function handleHelpAndVersion(argv: string[]): boolean {
...
function registerTools(server: McpServer, connectionManager: ConnectionManager) {
...
async function main() {
  ...
  for (const info of connected) {
    if (info.transport === "local") {
      ...
      console.error(
        `[INFO] Connected target "${info.name}": ${info.user}@${info.host}:${info.port} root=${info.workdir}`
      )
