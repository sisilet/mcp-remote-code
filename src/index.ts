...
async function main() {
  ...
  for (const startup of startupConnections) {
    if (!startup.workdir) {
      throw new Error(
        `Startup connection "${startup.name}" is missing a workdir. Provide --workdir or REMOTE_WORKDIR.`
      )
    }

    const info = await connectionManager.connectWithParams(
      startup.name,
      startup.sshCommand,
      startup.workdir,
      startup.password,
    ...
    console.error(
      `[INFO] Startup connection "${info.name}" ready: ${info.user}@${info.host}:${info.port} ${info.workdir}`
    )
  ...
  type ClientTransport = SSEServerTransport | StreamableHTTPServerTransport
  const transports = new Map<string, ClientTransport>()
