 ...
 */
const sftpNamespaceOk = new WeakMap<SSHPool, Promise<boolean>>()

function sftpMatchesShell(sshPool: SSHPool, _root: string): Promise<boolean> {
            ...
            .withSftp(
              (sftp: any) =>
                new Promise<string>((resolve) =>
                  sftp.realpath(".", (err: any, target: string) =>
                    resolve(err ? "" : String(target))
                  )
                )
            )
            .catch(() => ""),
        ])
        if (!shellPwd || !sftpPwd) return false
        const match =
          path.posix.normalize(shellPwd) === path.posix.normalize(sftpPwd)
        if (!match) {
          console.error(
            `[SSH] SFTP paths are offset from shell paths on this server ` +
 ...
 */
export async function remoteRealpath(
  sshPool: SSHPool,
  remotePath: string,
  root?: string
): Promise<string | null> {
  ...
  try {
    return await sshPool.withSftp(async (sftp: any) => {
      const exists = await new Promise<boolean>((resolve) => {
        ...
        sftp.lstat(remotePath, (err: any) => resolve(!err))
      })
      if (!exists) return null

      const resolved = await new Promise<string | null>((resolve) => {
        sftp.realpath(remotePath, (err: any, target: string) =>
...
async function remoteRealpathViaExec(
  sshPool: SSHPool,
  remotePath: string
): Promise<string | null> {
  const quoted = quoteShell(remotePath)
  const result = await sshPool.exec(
    `if [ -e ${quoted} ] || [ -L ${quoted} ]; then readlink -f ${quoted} 2>/dev/null || realpath ${quoted} 2>/dev/null || echo ${quoted}; else echo __MISSING__; fi`,
 ...
 * the batch.
 */
export async function resolveManyUnderRoot(
  root: string,
  inputs: string[],
  sshPool: SSHPool,
  options?: { forNewFile?: boolean; allowMissing?: boolean }
  ...
  if (await sftpMatchesShell(sshPool, root)) {
    ...
    return Promise.all(
      inputs.map((input, i) =>
        pre[i].error ? Promise.resolve(pre[i]) : resolveUnderRoot(root, input, sshPool, options)
      )
    )
  }
