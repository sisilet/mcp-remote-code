 ...
 */
export function createLocalPool(): SSHPool {
  return {
...
function createLocalSftpShim(): SFTPWrapper {
  const shim = {
    fastGet(remotePath: string, localPath: string, cb: (err?: Error | null) => void) {
      copyFile(remotePath, localPath).then(
        () => cb(null),
        (err) => cb(err)
      )
    },
    fastPut(localPath: string, remotePath: string, cb: (err?: Error | null) => void) {
      copyFile(localPath, remotePath).then(
        () => cb(null),
        (err) => cb(err)
      )
    },
  ...
  return shim as unknown as SFTPWrapper
}
